...
export function workspaceSummary(workspace) {
    const counts = workspace.nodes.reduce((acc, node) => {
    ...
    if (counts.code)
        parts.push(`${counts.code} code nodes`);
    if (counts.link)
        parts.push(`${counts.link} link nodes`);
    return parts.join(" • ");
}
export function selectedNodeSummary(node, workspace) {
    if (!node)
        return "Select a node to inspect its relationship surface.";
...
export function commandSuggestions(workspace) {
    const suggestions = [];
